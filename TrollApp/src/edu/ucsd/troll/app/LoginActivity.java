...
public class LoginActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		loginButton.setTypeface(btn_font);

		// Set Button Shape
		Drawable round_btn = getResources().getDrawable(R.drawable.round_btn);
		loginButton.setBackgroundDrawable(round_btn);
		...
		loginButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (username.trim().length() > 0
					...
					params.add(new BasicNameValuePair(TAG_APIKEYNAME,
							TAG_APIKEYVALUE));
	...
	private class LoginInUser extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... arg0) {
			...
			if (jsonStr != null) {
				try {
					...
					// user first name
					String responseUserFirstName = userObj
							.getString(TAG_FIRSTNAME);
					Log.d("first_name: ", "=> " + responseUserFirstName);
					// user last name
					String responseUserLastName = userObj
							.getString(TAG_LASTNAME);
					Log.d("last_name: ", "=> " + responseUserLastName);

					String responseFavorite = resultObj
							.getString(TAG_FAVORITES);
					Log.d("last_name: ", "=> " + responseFavorite);

					String responseUserToken = resultObj
							.getString("persist_code");
					Log.d("Persist_Code ", "=> " + responseUserToken);
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("success")) {
				...
				login.createLoginSession(user.get(TAG_ID),
						user.get(TAG_USERNAME), user.get(TAG_EMAIL),
						user.get(TAG_FIRSTNAME), user.get(TAG_LASTNAME),
